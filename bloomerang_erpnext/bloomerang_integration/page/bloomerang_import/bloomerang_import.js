frappe.pages['bloomerang_import'].on_page_load = function(wrapper) {
	let page = frappe.ui.make_app_page({
		parent: wrapper,
		title: __('Bloomerang Sync & Reconciliation'),
		single_column: true
	});

	// Render page layout into page wrapper
	$(frappe.render_template('bloomerang_import', {})).appendTo(page.main);

	// Add primary action button to Frappe Page action bar
	page.set_primary_action(__('Fetch Constituents'), function() {
		fetchBloomerangData(page);
	}, 'sync');

	// Bind click event to inline body button if present
	$(page.main).on('click', '#btn-fetch-constituents-body', function() {
		fetchBloomerangData(page);
	});
};

function fetchBloomerangData(page) {
	let $status = $(page.main).find('#bloomerang-status');
	let $container = $(page.main).find('#bloomerang-table-container');
